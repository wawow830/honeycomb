...
Every newly dispatched agent receives exactly `/skill:<specific-skill> <YAML_FILE>` as its initial prompt, using the input file's resolved absolute path.

Each skill except `honeycomb` and `idea` reads one YAML input package and writes one separate YAML output package. Honeycomb follows `idea` in its existing human conversation to produce the first YAML package. Supporting files and child-agent handoffs are allowed; no mandatory envelope or paired wrapper is required.
...
Resolve `^/path` from the project root in the current worktree and `=/path` from the production root, `^/.honeycomb/<production-id>/`. Resolve skill-local references from `.agents/skills/<skill-name>/`. These prefixes are notation, not shell expansion; resolve them before filesystem or adapter operations. Setup makes these conventions available to dispatched agents through the harness.

## Shared vocabulary

YAML structures inter-agent communication; shared dictionaries establish what its terms mean.

- The [Honeycomb dictionary](../.agents/skills/honeycomb/DICTIONARY.md) defines workflow terms. Skills reference this single source rather than copying it into each production.
...
Production artifacts live under `^/.honeycomb/<production-id>/`, also addressed as `=/`, where `<production-id>` is the ticket ID established during production setup.
...
Entry-point Honeycomb creates `=/pass-<n>/` before Validate and chooses the next unused pass number for each feedback iteration. It launches Validate through Human review with the destination pass directory as their working directory through the harness adapter. Output paths use `=/pass-<n>/`, where `<n>` is the destination pass number, not the input's pass number. Path roots do not change with the working directory. Only stages executed in that pass write outputs there. Unchanged inputs reference their existing artifacts in earlier passes rather than copying them.

```text
Production: `^/.honeycomb/task-123/`
...
Output: `=/pass-002/plan/plan.yaml`
```

Implementation worker outputs live in the same pass as their node inputs, mirroring `=/pass-<n>/plan/nodes/` under `=/pass-<n>/implement/nodes/`. Repair work gets new tracker nodes, rather than rewriting previous results.
...
Use `^/` for project references and `=/` for production references in YAML. References identify specific artifacts, not a mutable `latest.yaml` alias. The examples below use illustrative tracker IDs and repository paths.
...
2. Write the understood idea to `=/idea.yaml`.
...
`recon` writes `=/pass-<n>/validate/recon.yaml` containing `reconnaissance` and `verification`. `research` writes `=/pass-<n>/validate/research.yaml` containing `research`. `validate` uses their findings and the human discussion to produce its output below.
...
### Output: `=/pass-<n>/validate/validate.yaml`
...
4. Wait for both agents to stop, confirm normal completion, and read `=/pass-<n>/validate/recon.yaml` and `=/pass-<n>/validate/research.yaml`.
...
6. Write `=/pass-<n>/validate/validate.yaml`.
...
4. Write `=/pass-<n>/define.yaml` and link it from the existing production ticket, associating it with its specific `validate` input.
...
### Output: `=/pass-<n>/plan/plan.yaml`

```yaml
define: =/pass-001/define.yaml
...
nodes:
  - id: node-124
    ...
    references:
      - =/pass-001/define.yaml
      - =/pass-001/plan/plan.yaml
      - =/pass-001/validate/validate.yaml
...
7. Write `=/pass-<n>/plan/plan.yaml` and link it from the tracker nodes.
...
### Output: `=/pass-<n>/implement/implement.yaml`

```yaml
plan: =/pass-001/plan/plan.yaml
revision: <git commit>
evidence:
  - =/pass-001/implement/nodes/node-124.yaml
  - =/pass-001/implement/nodes/node-125.yaml
  - =/pass-001/implement/nodes/node-126.yaml
```
...
2. Assign open frontier nodes through the tracker. Extract each selected node into `=/pass-<n>/plan/nodes/<node-id>.yaml` in its input plan's pass, and start a separate `implement-worker` agent with that file through the project adapters. Run independent nodes concurrently.
...
5. Write `=/pass-<n>/implement/implement.yaml` with the `plan` reference, the complete implementation's Git commit as `revision`, and references to worker outputs as `evidence`.
...
### `implement-worker` input: `=/pass-<n>/plan/nodes/<node-id>.yaml`
...
### `implement-worker` output: `=/pass-<n>/implement/nodes/<node-id>.yaml`
...
### Output: `=/pass-<n>/qa.yaml`

```yaml
define: =/pass-001/define.yaml
implement: =/pass-001/implement/implement.yaml
findings:
...
4. Write `=/pass-<n>/qa.yaml` with findings explaining each shortcoming, its supporting evidence or reasoning, and its practical consequence.
...
4. Write `=/pass-<n>/review.yaml` with the human's findings.
...
4. Write `=/ship.yaml` and link it from the production ticket.
